...
try{
  ...
  if(isset($_REQUEST['status_residence']) && isset($_REQUEST['data_status'])){
    ...
    $status_incident_id = $con->real_escape_string(trim($_REQUEST['status_residence']));

    
    $sql_check_status = "SELECT status FROM incident_info WHERE incident_id = ?";
    $stmt_check_status = $con->prepare($sql_check_status) or die ($con->error);
    $stmt_check_status->bind_param('s',$status_incident_id);
    ...
    $sql_update_status = "UPDATE incident_info SET `status` = ? WHERE incident_id = ?";
    $stmt_update_status = $con->prepare($sql_update_status) or die ($con->error);
    $stmt_update_status->bind_param('ss',$data_status,$status_incident_id);
