...
try{
  ...
  if(isset($_REQUEST['id'])){
    ...
    INNER JOIN incident_status ON incident_record.blotter_id = incident_status.blotter_main
    INNER JOIN incident_complainant ON incident_record.blotter_id = incident_status.blotter_main WHERE incident_record.blotter_id = ?";
...
<!-- Modal -->
<div class="modal " id="viewResidentRecordModal" data-backdrop="static" data-keyboard="false" tabindex="1" >
                ...
                <div class="col-sm-12">
                          ...
                          while($row_record_resident_id = $result_resident_id->fetch_assoc()){
                            ...
                            ?>
                              <option  value="<?= $row_record_resident_id['residence_id'] ?>" <?php
                                ...
                                INNER JOIN incident_status ON incident_record.blotter_id = incident_status.blotter_main
                                INNER JOIN incident_complainant ON incident_record.blotter_id = incident_status.blotter_main WHERE incident_complainant.blotter_main = ?";
                                $stmt_record_while_complainant = $con->prepare($sql_record_while_complainant) or die ($con->error);
                                $stmt_record_while_complainant->bind_param('s',$id);
                  ...
                  <div class="col-sm-12 ">
                    <div class="form-group form-group-sm">
                      ...
                      <textarea name="edit_complainant_not_residence"  id="edit_complainant_not_residence" cols="57" disabled class="bg-transparent text-white form-control"><?= $row_record_blotter['complainant_not_residence'] ?></textarea>
                    </div>
                  </div>
                  <div class="col-sm-12 ">
                    <div class="form-group form-group-sm">
                      <label>Complainant Statement</label>
                      <textarea name="edit_complainant_statement" id="edit_complainant_statement" cols="57" disabled rows="3" class="bg-transparent text-white form-control"><?= $row_record_blotter['statement'] ?></textarea>
                    </div>
                  </div>
                  <div class="col-sm-12 ">
                    <div class="form-group form-group-sm">
                      <label>Respodent</label>
                        <input name="edit_respodent" value="<?= $row_record_blotter['respodent'] ?>" id="edit_respodent" disabled  class=" form-control">
                    </div>
                  </div>
                  <div class="col-sm-12">
                    <div class="form-group form-group-sm">
                        <label>Person Involved Resident</label>
                      <select name="edit_person_involed[]" multiple="multiple" id="edit_person_involed" class="select2bs4" disabled style="width: 100%;">
                        <option></option>
                        <?php 
                          ...
                          while($row_person_id = $result_person_id->fetch_assoc()){
                            ...
                            ?>
                              <option value="<?= $row_person_id['residence_id'] ?>" <?php 
                                    ...
                                    INNER JOIN incident_status ON incident_record.blotter_id = incident_status.blotter_main
                                    INNER JOIN incident_complainant ON incident_record.blotter_id = incident_status.blotter_main WHERE incident_status.blotter_main = ?";
                                    $stmt_record_while_person = $con->prepare($sql_record_while_person) or die ($con->error);
                                    $stmt_record_while_person->bind_param('s',$id);
